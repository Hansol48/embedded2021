...
int findBuzzerSysPath(){
	DIR 8 dir_info = opendir(BUZZER_BASE_SYS_PATH);
	int ifNotFound = 1;
	if (dir_info != NULL){
		while (1){
			...
			if (strncasecmp(BUZZER_FILENAME, dir entry->d_name, strlen(BUZZER_FILENAME)) == 0){
				ifNotFound = 0;
...
{
	char path[200];
	sprintf(path, "%s%s", gBuzzerBaaseSysDir, BUZZER_FREQUENCY_NAME);
	int fd=open(path, O_WRONLY);
